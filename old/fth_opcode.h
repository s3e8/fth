...
BYTECODE(TICK, "'", 0, 0, FLAG_HASARG|FLAG_IMMED, {
    DBG_PRINT("TICK: reading word for tick...\n")
    read_word(inputstate, wordbuf);
    DBG_PRINT("TICK: word='%s'\n", wordbuf);
    word_hdr_t *de = find_word(wordbuf);
    if(!de) {
        DBG_PRINT("TICK: word not found!\n");
        exit(1);
    }
...
BYTECODE(INTERPRET, "interpret", 0, 0, 0, {
    ...
    DBG_PRINT("INTERPRET: word='%s' with state: %ld\n", word ? word : "(null)", state);
    ...
    if(!entry) {
      ...
      if(*endptr!='\0') {
        DBG_PRINT("  ERROR: unknown word\n");
	      printf("ERROR: no such word: %s\n", word);
      } else {
        DBG_PRINT("  parsed number: %ld\n", val);
    ...
    if(state == STATE_COMPILE && !(entry->flags & FLAG_IMMED)) {
        ...
        void **code = cfa(entry);
      *--nestingstack = ip;
      if(nestingstack < nestingstack_space) {
      ...
      if(entry->flags & FLAG_BUILTIN) {
        ...
        DBG_PRINT("Setting up word_immediatebuf for word: %s\n", entry->name);
	      word_immediatebuf[1] = (void*)code;
	      ip = word_immediatebuf;
      }
    }
